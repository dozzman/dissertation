...
\end{lstlisting}
\hfill\\
The code above gives an example of using the Java.make and Java.call functions. Two Java objects are created named `obj'\todo{don't quote these things, wrap them using texttt} and `itg' using different classes via the Java.make
...
Ubuntu version something\todo{find pwf facilities version}.
...
taking threads off the yield queue and executing them. Listing blablabla\todo{find out how to reference listings} summarises the behaviour of a worker thread. Initially the OCaml thread (that is, the first thread executing the main
...
%
As an example, listing blablabla\todo{insert that code i wrote just now in here soon} is code that when run, simply outputs what thread it is running on. It does this 20 times before quitting, leaving a trace of all the threads that
...
between the worker threads as to which one picks up the yield thread, therefore the output from running this program is different each time, demonstrating real parallel asynchonous execution.\todo{this is slightly misleading, might
just remove it}
...
%
Sustituting a global runtime lock for a coarse grained locking implementation allows the project to take full advantage\todo{`full' or `more'?} advantage of the parallelism exposed by OCaml-Java. The aim of the coarse grained lock is
...
\item The locks should do something more\todo{rule of three, need one more here}
\end{enumerate}
%
...
of redirection within a reference to a thread -- as depicted by figure~\ref{fig:thread_repr}\todo{write a proper caption}.
\begin{figure}[h!]
\includegraphics[scale=0.75]{thread_repr}
...
A visual description of the problem is outline in figure~something\todo{draw best representation of this problem as possible} along with the offending code segment in listing~something~else\todo{add bind code somewhere here}. 
\begin{figure}[h!]
\includegraphics{bind_callback}
\caption{insert descriptive caption here}
...
picked up by a worker thread to begin execution. A visual description of this change is outlined in figure~something\todo{draw new diagram of locked thread being bound and then unlocked later on, possibly (seems hard to draw).}
...
OCaml using Java's high-resolution time source via java.lang.System.nanoTime, in fact within the Java documentation, example code shows the use of nanoTime in determining how long a code segment took to run. Listing~something\todo{add
listing for .mli of timer module} shows some of the commands used within the simple timer module.
...
section~\ref{sec:parallelism_overview}. Figure blablabla shows the state of the system in the middle of the algorithm\todo{do this figure, maybe idk it seems a bit useless}.

%
% Wrote fibonacci benchmark (give example code, and talk about how it would run in the system)
%
Another choice of benchmark which was both easy to implement and also featured within the Introduction to Algorithms book was the recursive fibonacci algorithm adapted for multithreading.\todo{say something more about fib sort
here}
...
system that exist\todo{link camlp4 bugs in mantis bug tracker}, therefore these files can be manually preprocessed using the default Camlp4 preprocessor which will produce the same output.
...
\todo{add help text of run\_tests.sh}
...
The performance results of the merge sort algorithm are shown in the speed up graph in figure bla\todo{add merge sort speedup graph}, where the x~axis gives the number of cores in use and the y~axis is the speed up, calculatated as
$t_1 / t_n$ where $t_1$ is the time taken to perform the sort on 1 thread and $t_n$ is the time taken to execute the sort on n threads. There is a clear, almost linear speed up between using 0 and 8 cores for the sort, reaching up to
...
T(n) = 
    \begin{cases}
    2T\left({n \over 2}\right) + \Theta(n) & \text{if } n > 0 \\
    1 & \text{if } n = 0
    \end{cases}
\]
...
Therefore in fact we are expecting to see a logarithmic shaped graph as our speed up graph, which is exactly what figure~bla\todo{reference lwt merge sort once inserted} shows us. Figure~blabla\todo{insert vanilla parallel merge sort
graph} shows a merge sort written in vanilla Java with the same test parameters and the speed up results obtained from these tests. This mirrors the results obtained from using the interface which is a very good sign to suggest that
the interface is performing well. The vanilla Java merge sort reaches a peak speed up of around 5 whereas the interface provides a peak speed up between 3 and 4 which is a confident result for both the OCaml-Java compiler and the
interface.
...
%
The results of performance testing recursive parallel fibonacci are displayed in figure blablabla\todo{add fibonacci increasing number graph}. This graph describes the execution time of the fibonacci algorithm where the x~axis denotes
the fib number being calculated and the y axis denotes the time taken to calculate that fibonacci number. The graph clearly shows that calculating fibonacci on multiple cores actually performs worse than using the same recursive
algorithm on a single core. This is highly counter intuitive under the assumption that more cores means more power. Although there are many more cores executing the fibonacci calculations, it would seem the overhead of managing many
LWT threads is significant enough to completely negate any potential speed up gains from utilising multiple cores.
...
%
Overall the fibonacci calculation has not scaled well with the 
