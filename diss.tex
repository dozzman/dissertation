...
The functions to be executed by the yield threads will most likely add more threads to the yield queue. These threads are added to the bottom of the queue and are not executed until the next polling iteration. Figure blabla\todo{draw
a diagram of thread queue being executed} describes the functionality of the yield queue graphically.
%
...
threads off the yield queue and executing them. Listing blablabla\todo{find out how to reference listings} summarises the behaviour of a worker thread. Initially the OCaml thread (that is, the first thread executing the main OCaml
...
processing has been finished and the worker thread is about to enter its next iteration of work. All other threads are blocked, waiting on the runtime lock. Figure blablabla\todo{draw a picture of the naive runtime lock situation}
...
of redirection within a reference to a thread -- as depicted by figure~something\todo{draw figure of layers of thread\_repr's}. Therefore it is important to ensure the behaviour of a thread lock is consistent and sensible when dealing
...
A visual description of the problem is outline in figure~something\todo{draw best representation of this problem as possible} along with the offending code segment in listing~something~else\todo{add bind code somewhere here}. The
...
alternatives. These are highly configurable and complex software suites which allow a lot of configuration -- most of which I wouldn't need. For the purpose of profiling this project, I would simply need an accurate timer to determine
the time taken to complete a function so that these times may be compared and the values used in speed up calculations in chapter~\ref{chap:evaluation}. Therefore I wrote a very simple timer module in OCaml using Java's
high-resolution time source via java.lang.System.nanoTime, in fact within the Java documentation, example code shows the use of nanoTime in determining how long a code segment took to run. Listing~something\todo{add listing for .mli
of timer module} shows some of the commands used within the simple timer module.
...
%
Choosing an appropriate benchmark which exploits parallelism well was important to demonstrate the true capabilities of the system. One of my choices for parallel benchmarking was merges sort after investigating the Barcelona OpenMP
Task Suite -- one of their benchmarks is an integer sort algorithm based on the Cilk sorting algorithm. The Cilk project is a suite for algorithmic multithreaded programming and the Cilk sort performs a mixture of merge sort and
insertion sort for arrays smaller than a certain threshold. Furthermore the classic Introductions to Algorithms book \cite{coren2009} uses merge sort as an introduction to algorithms that exploit parallelism well.  is a mixture of a
merge and insertion sort. The mergesort algorithm implemented in OCaml is essentially a mirror of the Cilk algorithm with the exception that the insertion sort in Cilk has been replaced with a simple call to OCaml's default array
sorting algorithm `Array.sort'.

Another choice of benchmark which was both easy to implement and also featured within the Introduction to Algorithms book was the recursive fibonacci algorithm written adapted for multithreading.
